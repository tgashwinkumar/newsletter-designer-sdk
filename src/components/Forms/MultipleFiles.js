...
const MultipleFiles = ({
  ...
  useEffect(() => {
    ...
    let currentFiles = files.filter(file => (pdf && file.type === "application/pdf") ||
      (!pdf && (file.type === "image/jpeg" || file.type === "image/jpg" || file.type === "image/png")));
    if (initialFileLen === currentFiles.length) return;
    else {
      setFiles(currentFiles);
      pdf ? toast.error("Please upload valid PDF files") : toast.error("Please upload valid image files");
    }
  }, [files])
...
const FileItem = ({ file, url = "", onRemove }) => {
  ...
  return (
    <div className="flex items-center space-x-2 relative">
      {((url && url.length > 0 && (url.endsWith(".png") || url.endsWith(".jpg") || url.endsWith(".jpeg"))) ||
        file && (
        file.type === "image/png" ||
        file.type === "image/jpg" ||
          ...
          <React.Fragment>
            ...
            <div>
              <Button
                ...
                handleClick={() => {
                  url.length > 0 ? window.open(url, "_blank") : window.open(URL.createObjectURL(file), "_blank");
                }}
              />
            </div>
          </React.Fragment>
